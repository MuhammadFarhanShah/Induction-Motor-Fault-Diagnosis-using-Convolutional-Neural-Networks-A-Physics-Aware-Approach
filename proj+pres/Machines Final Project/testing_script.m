...
%% 1. User Inputs (Update Variable Names Here)
try
    data_Ia = Ia; 
    data_Ib = Ib;
    data_Ic = Ic;
...
%% 2. Configuration (Must Match Training exactly)
Fs = 1000;
window_size = 30000;      % 3s
spec.window = 2048;      % MATCH YOUR TRAINING SCRIPT
spec.noverlap = 1536;     % MATCH YOUR TRAINING SCRIPT
spec.nfft = 4096;        % MATCH YOUR TRAINING SCRIPT
img_size = [224, 224];
transient_samples = 10000; % 1.0 second at 10kHz

% *** ADDED: Noise parameter to match training ***
noise_std_frac = 0.01;   
...
%% 4. Preprocess Signal (CUT TRANSIENT LOGIC)
...
% *** ADDED: Apply Noise to match Training Conditions ***
...
function img = create_spectrogram_local(Ia, Ib, Ic, Fs_target, img_size, window, noverlap, nfft)
    ...
    % --- FIX 1: REMOVE DC OFFSET (Matches Training) ---
    ...
    S_db_all = [];
    for ch = 1:3
        % --- FIX 2: USE CHEBYSHEV WINDOW (Matches Training) ---
